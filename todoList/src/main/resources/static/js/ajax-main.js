...
function getCompleteCount() { // 함수 정의
    ...
    // 두 번째 then()의 result : 
    // 첫 번째 then에서 알맞게 데이터(text(), json()) 가 변환된 응답 데이터
...
addBtn.addEventListener("click", () => {
    ...
    // - 전달 데이터 : todoTitle.valu , todoContent.value
    // JSON
    // { "name" : "홍길동",
    //    "age" : 20,
...
const selectTodoList = () => {
    ...
    .then(todoList => {
        ...
        console.log(todoList); // JS객체 배열 형태로 출력
...
deleteBtn.addEventListener("click", () => {
    ...
    fetch("/ajax/delete", {
        ...
        body : todoNo // todoNo 단일 값 하는 JSON 형태로 자동 변환되어 전달됨
...
changeComplete.addEventListener("click", () => {
    ...
    .then(result => {
        ...
        if(result > 0) { // 성공
            ...
            selectTodoList();
            // 서버 부하 줄이기 가능! -> 하지만 코드가 복잡해서 그냥 사용
...
updateBtn.addEventListener("click", e => {
    ...
    .then(result => {
        ...
        if(result > 0) {
            ...
            updateBtn.removeAttribute("date-todo-no"); // 
