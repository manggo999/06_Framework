...
if(updateInfo != null) {
    ...
    updateInfo.addEventListener("submit", async (e) => {
        ...
        // 기존 닉테임과 새로 입력된 닉네임이 다르면 중복검사 시도하기
        // -> 같으면 변경된 적 없다 -> 중복검사 진행안함
        if(currentNickname !== memberNickname.value) {

            // 비동기 요청 (fetch() API 이용)
            // async / await 사용
            // async : 비동기 함수를 만들 때 사용하는 키워드("이 함수 내에는 오래 걸리는 작업이 있어요!")
            // await : 비동기 작업의 결과를 기다릴때 사용 - > 반드시 async 함수 안에서만 사용 가능!!
...
imageInput.addEventListener("change", () => {
    ...
    if(file) {  // 파일이 선택된 경우
        if(file.size <= MAX_SIZE) { // 파일 크기가 허용범위 이내인 경우
            ...
            previousImage = newImageUrl; // 현재 선택된 이미지를 이전 이미지로 저장(다음에 바뀔 일 대비)
            previousFile = file; // 현재 선택된 파일 객체를 이전 파일로 저장(다음에 바뀔 일 대비)
            ...
            if(previousFile) { 
                ...
                //  DataTransfer 를 이용해여 현재 File 객체를 FileList만 변환하여 할당
        ...
        if(previousFile) {
            const dataTransfer = new DataTransfer();
            dataTransfer.items.add(previousFile);
            imageInput.files = dataTransfer.files; // 이전 파일로 input 태그의 files 속성
        }
...
deleteImage.addEventListener("click", () => {
    ...
    if(profileImg.src !== defaultImageUrl) {
        ...
        profileImg.src = defaultImageUrl;  // 기본이미지로 설정
