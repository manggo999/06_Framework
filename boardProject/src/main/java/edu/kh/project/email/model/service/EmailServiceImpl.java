...
public class EmailServiceImpl implements EmailService{
	...
	// 필드 의존성 주입될 객체들
	private final EmailMapper mapper; // EmailMpper 의존성 주입
	private final JavaMailSender mailSender;// JavaMailSender : 실제 메일 발송을 담당하는 객체 (EmailConfig 설정이 적용된 
	private final SpringTemplateEngine templateEngine; // SpringTemplateEngine : 타임리프(템플릿 엔진)를 이용해서 html 코드 -> 
	...
	public String sendEmail(String htmlName, String email) {
		...
		try {
			...
			// mimeMessage : MimeMessage 객체로, 이메일 메세지의 내용을 담고있음
	...
	private String loadHtml(String authKey, String htmlName) {
		// 
		// Context : 타임리프에서 제공하는 HTML 템플릿에 데이터를 전달하기 위해 사용하는
	...
	// 인증번호 발급 메서드
	// UUID를 사용하여 인증키 생성
	// Universally Unique IDentifier : 전 세계에서 고한 식별자를 생성하기 위한 표준
