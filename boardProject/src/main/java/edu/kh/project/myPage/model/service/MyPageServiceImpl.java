...
public class MyPageServiceImpl implements MyPageService{
	...
	public int changePw(Map<String, Object> paramMap, int memberNo) {
		...
		// 입력받은 현재 비밀번호와 DB에서 조회한 비밀번호를 비교
	...
	public String fileUpload(MultipartFile uploadFile) throws Exception {
		...
		// - transferTo(경로) : 
		//   메모리 또는 임시 저장 경로에 업도드된 파일을 원하는 경로에 실제로 전송
		//   (서버 어떤 폴더에 저장할지 지정)
	...
	public int fileUpload2(MultipartFile uploadFile, int memberNo) throws Exception {
		...
		// log.debug("fileRename : " + fileRename); // 20241112101913_00001.JPG
		...
		uploadFile.transferTo(new File(folderPath + fileRename));
						// C:/uploadFiles/test/20241112101913_00001.JPG
	...
	// 여러 파일 업로드	
	@Override
	public int fileUpload(List<MultipartFile> aaaList, List<MultipartFile> bbbList, 
